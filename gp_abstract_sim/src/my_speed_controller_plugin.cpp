  ...
  void OnUpdate()
  {
    ...
    if (delta > max_delta && delta != 0.0)
    {
      ...
      else
      {
        ...
        this->model->GetJoint("face_mover_joint")->SetLowerLimit(0, 0.0);
        this->model->GetJoint("face_mover_joint")->SetUpperLimit(0, 1.5717);
        this->model->GetJoint("face_mover_joint")->SetParam("fmax", 0, 50.0);
        ...
        if ((new_secs - this->startTime) > 10.0)
        {
          ...
          if ((this->model->GetName() == "dittaya") && (this->counterflag == 0))
          {
            ...
            this->model->GetJoint("face_mover_joint")->SetParam("vel", 0, 0.1);
          }
          // this->model->GetLink("front_face")->GetInertial()->SetMass(150.0);
          // this->model->GetLink("front_face")->UpdateMass();
          this->model->GetJoint("face_mover_joint")->SetParam("vel", 0, front_face_speed_magn);
        }
      }
    }
  }
